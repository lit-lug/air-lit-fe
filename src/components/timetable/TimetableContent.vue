...
function handleTouchEnd() {
  ...
  if (towardsX.value > 50) {
    if (currentWeekIndexTemp === 0) return;
    currentWeekIndexTemp--;
  } else if (towardsX.value < -50) {
    if (currentWeekIndexTemp === weekNum.value - 1) return;
    currentWeekIndexTemp++;
  }
...
<template>
  ...
  >
    ...
    <div
      class="bg-blue-5/80 fixed top-40% z-30 rounded-l-full transition-all duration-300"
      text="white sm"
      p="l-4 y-2 r-2"
