...
const isCurrentWeek = (weekIndex: number) => {
  ...
  if (
    !originalWeekIndex.value ||
    !currentWeekIndex.value ||
    !originalWeekWeekIndex.value
  )
    return false;
  return (
...
<template>
  <div
    class="h-10 shadow-sm px-1 top-0"
    grid="~ cols-[0.7fr_repeat(7,1fr)] gap-1"
  >
    ...
    <div
      ...
      :class="
        isCurrentWeek(index) ? 'bg-light-blue-300 !b-b-light-blue-500' : ''
      "
    >
      <p class="font-medium">
        {{ weekTitle[index] }}
